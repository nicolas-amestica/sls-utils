...
module.exports.uploadFiles = async (fileName, filePath) => {
    ...
    try {
        ...
        let result = await S3.upload(fileName, filePath, process.env.BUCKET_LIQ_PROC_3);

        if (result.error)
            throw result.error;
            
        result.Name = `ventasLiquidadas_${dateFormat(new Date(), "yyyymmddHMM")}.csv`;
